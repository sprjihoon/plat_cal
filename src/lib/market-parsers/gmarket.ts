...
const GMARKET_ORDER_HEADERS = [
  '주문번호', '주문일', '결제일',
  '상품명', '옵션', '수량', '판매가',
  '주문상태', 'G마켓',
];

const GMARKET_SETTLEMENT_HEADERS = [
  '정산예정일', '주문번호', '상품명', '판매금액',
  '수수료', '정산금액',
];

export const gmarketParser: MarketParser = {
  name: 'G마켓/옥션',
  channel: 'gmarket',

  detect(headers: string[]): boolean {
    const headerStr = headers.join(' ');
    const hasGmarket = headerStr.includes('G마켓') || headerStr.includes('지마켓') ||
                       headerStr.includes('옥션') || headerStr.includes('Gmarket') ||
                       headerStr.includes('ESM');
    const headerSet = new Set(headers.map(h => h.trim()));
    const hasEsmFields = headerSet.has('장바구니번호') || headerSet.has('사이트') || headerSet.has('원주문번호');
    const orderMatch = GMARKET_ORDER_HEADERS.filter(h => headerSet.has(h)).length;
    const settlementMatch = GMARKET_SETTLEMENT_HEADERS.filter(h => headerSet.has(h)).length;
    return hasGmarket || hasEsmFields || orderMatch >= 4 || settlementMatch >= 3;
  },

  parseOrders(data: Record<string, string>[]): MarketOrderRow[] {
    return data.map(row => {
      const site = (row['사이트'] || '').toLowerCase();
      const channel = site.includes('옥션') || site.includes('auction') ? 'auction' : 'gmarket';

      const unitPrice = parseNumber(row['판매가'] || row['판매단가'] || row['상품가격']);
      const quantity = parseNumber(row['수량'] || row['주문수량']) || 1;
      const totalAmount = parseNumber(row['결제금액'] || row['주문금액'] || row['상품금액']) || unitPrice * quantity;
      const platformFee = parseNumber(row['수수료'] || row['판매수수료']);
      const shippingFee = parseNumber(row['배송비']);

      return {
        orderId: row['주문번호'] || row['장바구니번호'] || row['원주문번호'] || '',
        orderDate: parseDate(row['주문일'] || row['주문일시'] || row['결제일']),
        productName: row['상품명'] || row['아이템명'] || '',
